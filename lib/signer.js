...
function sign(source) {
  ...
  var signedPackageText = util.format('/*@%s %s*/\n', 'generated', ssToken) + source

  return signCode(signedPackageText).signed_data
}
...
exports.signingToken = signingToken
...
function signCode(file_data) {
  var first_time = file_data.indexOf(TOKEN) !== -1

  if (!first_time) {
    if (isSigned(file_data))
      file_data = file_data.replace(PATTERN, signingToken())
    else
...
function verifySignature(file_data) {
